...
test.describe('SYS Verbindung', () => {
  ...
  test('MT5-Scanner und eigener Pfad', { tag: '@SYS-04' }, async ({ page, worker, dashboard }) => {
    ...
    await expect(select.locator('option')).toHaveText([
      msg('account.path.select.placeholder'),
      MT5_PATH,
    ...
    await expect(dialog.getByLabel(msg('account.path.custom.aria'))).toBeVisible();

    // Scan-Fehler: Hinweis + Textfeld für den Pfad
    await dialog.getByLabel(msg('account.path.custom')).uncheck();
    worker.overrides.set('GET /api/system/scan-mt5', { status: 500, body: { detail: 'boom' } });
    await dialog.getByRole('button', { name: msg('account.path.rescan') }).click();
    await expect(dialog.getByText(msg('account.path.scanFailed'), { exact: false })).toBeVisible();
    await expect(dialog.getByLabel(msg('account.path.custom.aria'))).toBeVisible();
  });
  ...
  test('Gespeicherter Pfad außerhalb der Scan-Liste öffnet „eigener Pfad“', { tag: '@SYS-04' }, async ({ page, worker, dashboard }) => {
    ...
    await expect(dialog.getByLabel(msg('account.path.custom.aria'))).toHaveValue('D:/Portable/MT5/terminal64.exe');
  });
});
...
test.describe('UPD System', () => {
  test('Update-Prüfung', { tag: '@UPD-01' }, async ({ page, worker, dashboard }) => {
    ...
    await expect(modal.getByRole('button', { name: msg('update.apply') })).toBeVisible();
  });
  ...
  test('Update anwenden lädt die Seite neu', { tag: '@UPD-02' }, async ({ page, worker, dashboard }) => {
    ...
    await page.getByRole('button', { name: msg('update.apply') }).click();
    await reloaded;
