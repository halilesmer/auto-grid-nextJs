...
test.describe('ACC Konten', () => {
  test('Kontoliste zeigt alle Konten', { tag: '@ACC-01' }, async ({ page, dashboard }) => {
    ...
    await expect(dashboard.accountSelect).toHaveText('-- Select an account --');
  ...
  test('Doppelter Login: Rückfrage, kein zweites Konto', { tag: '@ACC-03' }, async ({ page, worker, dashboard }) => {
    ...
    const question = page.getByRole('dialog').filter({ hasText: 'Account already exists' }).last();
    ...
    await expect(question).toContainText('(Login: 1001) already exists. Edit it instead?');
